...
score_twitter <- function(data, hashtags){
  # data<-tweetFeed
  # hashtags<-c("ltc", "litecoin", "blockchain")
  ...
  dataAgg<-aggregate(. ~ cut(data$created_at, paste(as.character(90),"min")), 
  ...
  for (i in (nrow(dataAgg)-(8+lag+1)):nrow(dataAgg)){
  ...
  dataAgg[(nrow(dataAgg)-(12-1)):nrow(dataAgg),c(1,6)]
}
...
dfResults<-score_twitter(tweetFeed, c("ltc", "litecoin", "blockchain"))
...
dfResults2<-data.frame(scores[,1])
dfResults2[,2]<-"NA"
dfResults2[,3]<-scores[,2]
names(dfResults2)<-c("tStamp","score","sentiment")

places<-rev(c(1,3,4,6,7,9,10,12))
n=0
for (i in places){
  dfResults2$score[i]<-dfResults$score[nrow(dfResults)-n]
  n=n+1
}

write.csv(dfResults2, file = "twitterScore.csv")
