...
score_twitter <- function(data, hashtags){
  # data<-tweetFeed
  # hashtags<-c("xrp", "ripple", "blockchain")
  ...
  dataAgg[(nrow(dataAgg)-(74-1)):nrow(dataAgg),c(1,6)]
}
...
write.csv(score_twitter(tweetFeed, c("xrp", "ripple", "blockchain")),"resultsTwitterScore.csv")
