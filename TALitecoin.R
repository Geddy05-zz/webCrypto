list.of.packages <- c("mongolite", "TTR","quantmod","caret","pROC","FSelector","mlbench")
new.packages <- list.of.packages[!(list.of.packages %in% installed.packages()[,"Package"])]
if(length(new.packages)) install.packages(new.packages)

library(mongolite);library(quantmod); library(TTR); library(caret);library(pROC);library(FSelector);library(mlbench)
...
currencyResult <- dbCon$find(paste0('{"tags":"Litecoin","ticks.last_updated": {"$gte": "', minDate, '" , "$lte": "',maxDate,'"}}'))
